...
描述
	...
	从左至右，分别乘以1，2，…，9，再求和，即0×1+6×2+……+2×9=158，
	然后取158 mod 11的结果作为识别码。
...
int main()
{
    char arr[13] = {};//将ISBN号码转化为字符
    scanf("%s", arr);//输入相应的ISBN号

    int res = 0;//计算最后识别码的结果
    ...
    for (i = 0, j = 1; i < 11; i++)
    {
        if (arr[i] != '-')
        {
            res += (arr[i] - '0') * j;//累加到res
            j++;
        }
    }

    int m = res % 11;//m储存res模后的值

    //如果m和对于的识别码相等或 m为10且识别码为X
    if (arr[12] - '0' == m || (m == 10 && arr[12] == 'X'))
    {
        printf("Right\n");
    }
    else
    {
        if (m == 10)
        {
            arr[12] = 'X';//如果m为10，则识别码应该为X
            printf("%s\n", arr);
        }
        else
        {
            arr[12] = m + '0';//m为其他数,转化为对应的字符
            printf("%s\n", arr);
        }
    }
	return 0;
